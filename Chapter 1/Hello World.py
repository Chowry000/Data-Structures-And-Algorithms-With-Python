...
#Accessor and mutator methods- Accessor methods does not change the original
#object. While mutator methods change the object. For example:

x= "i will demonstrate accessor object"
y = x.upper()
print('x:',x,'y:',y)

# It can be seen that the upper() did not change the original object x

lista = ['object', 'mutator', 'demonstrate', 'will', 'I']
lista.reverse()
print(lista)

# It can be seen here that the mutator method changes the original list
# Once called, a mutator method can’t be undone. The change or mutation
# is permanent until mutated again by some other mutator method.

# When a class does not contain any mutator methods, we say that the class
#is immutable. For ex: the 'str' class
